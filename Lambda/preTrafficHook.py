...
def handler(event, context):
    ...
    try:
      response = client.put_lifecycle_event_hook_execution_status(
          deploymentId=params['deploymentId'], lifecycleEventHookExecutionId=params['lifecycleEventHookExecutionId'], status=params['status'])
      print(
          "putLifecycleEventHookExecutionStatus done. executionStatus=[" + params['status'] + "]")
